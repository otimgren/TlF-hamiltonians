...
def test_stark_spectra(H_EB_coupled, H_EB_uncoupled, H_EB_coupled_P, QN_coupled):
    """
    ...
    energies_coupled = np.empty((len(QN_coupled), len(Es)))
    energies_uncoupled = np.empty((len(QN_coupled), len(Es)))
    energies_coupled_P = np.empty((len(QN_coupled), len(Es)))
    for i, E in enumerate(Es):
        energies_coupled[:,i], _ = np.linalg.eigh(H_EB_coupled(np.array([0,0,E]), np.array([0,0,0])))
        energies_uncoupled[:,i], _ = np.linalg.eigh(H_EB_uncoupled(np.array([0,0,E]), np.array([0,0,0])))
        energies_coupled_P[:,i], _ = np.linalg.eigh(H_EB_coupled_P(np.array([0,0,E]), np.array([0,0,0])))

    message1 = "Stark spectrum not the same for coupled and uncoupled"
    assert np.allclose(energies_coupled, energies_uncoupled), message1

    message2 = "Stark spectrum not the same for coupled and coupled_P"
