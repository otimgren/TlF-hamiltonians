...
class TestHSx:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HSx in B_coupled_P is not Hermitian!"
...
class TestHSy:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HSy in B_coupled_P is not Hermitian!"
        assert np.allclose(M, M.conj().T), message
...
class TestHSz:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HSz in B_coupled_P is not Hermitian!"
        assert np.allclose(M, M.conj().T), message
...
class TestIsotropic:
    def test_spectrums_same(self, QN, H0):
        ...
        message_xy = "Stark effect not isotropic for B_coupled_P: x!=y"
        assert np.allclose(energies_x, energies_y), message_xy

        message_xz = "Stark effect not isotropic for B_coupled_P: x!=z"
