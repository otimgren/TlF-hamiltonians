...
class TestHZx:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HZx in B_coupled is not Hermitian!"
...
class TestHZy:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HZy in B_coupled is not Hermitian!"
        assert np.allclose(M, M.conj().T), message
...
class TestHZz:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HZz in B_coupled is not Hermitian!"
        assert np.allclose(M, M.conj().T), message
...
class TestIsotropic:
    def test_spectrums_same(self, QN, H0):
        ...
        message_xy = "Zeeman effect not isotropic for B_coupled: x!=y"
        assert np.allclose(energies_x, energies_y), message_xy

        message_xz = "Zeeman effect not isotropic for B_coupled: x!=z"
