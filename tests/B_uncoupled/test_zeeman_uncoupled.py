...
from TlF_hamiltonians.B_uncoupled import (H_c1p, H_mhf_F, H_mhf_Tl, H_q, Hc1,
                                          Hrot, HZx, HZy, HZz)
from TlF_hamiltonians.B_uncoupled.quantum_numbers import generate_QN
...
class TestHZx:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HZx in B_uncoupled is not Hermitian!"
...
class TestHZy:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HZy in B_uncoupled is not Hermitian!"
        assert np.allclose(M, M.conj().T), message
...
class TestHZz:
    ...
    def test_hermitian(self, QN):
        ...
        message = "HZz in B_uncoupled is not Hermitian!"
        assert np.allclose(M, M.conj().T), message
...
class TestIsotropic:
    def test_spectrums_same(self, QN, H0):
        ...
        message_xy = "Zeeman effect not isotropic for B_uncoupled: x!=y"
        assert np.allclose(energies_x, energies_y), message_xy

        message_xz = "Zeeman effect not isotropic for B_uncoupled: x!=z"
