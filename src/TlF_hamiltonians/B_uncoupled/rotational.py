...
def J2(psi: UncoupledBasisState) -> State:
    return State([(psi.J*(psi.J+1),psi)])

def J4(psi: UncoupledBasisState) -> State:
    return State([( (psi.J*(psi.J+1))**2, psi)])

def J6(psi: UncoupledBasisState) -> State:
    return State([( (psi.J*(psi.J+1))**3, psi)])

@state_operator
def Hrot(psi: UncoupledBasisState, B: float = cst_B.B_rot_B, 
